...
def build_report(results_csv: Path, pdf_out: Path) -> None:
    ...
    elems.append(Paragraph("Dataset Origin", styles["Heading2"]))
    elems.append(
        Paragraph(
            "The datasets used in this work were not artificially generated. Both datasets—Breast Cancer Wisconsin (Diagnostic) and Heart Failure Clinical Records—come from the UCI Machine Learning Repository and are widely used in academic and scientific studies. In the case of Breast Cancer, the data are provided directly by the scikit-learn library, maintaining their origin and integrity.",
            styles["BodyText"],
        )
    )
