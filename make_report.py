...
def main():
    ...
    for dataset in sorted(df["dataset"].dropna().unique()):
        ...
        if dataset.lower().startswith("breast"):
            ...
            story.append(Paragraph("O conjunto de dados Heart Failure mostrou-se muito mais difícil de lidar. Embora os valores de accuracy sejam aceitáveis, ao analisar as matrizes de confusão, observou-se um número considerável de falsos negativos, especialmente nos casos com 30% e 50% de dados de teste. Isso sugere que os modelos têm dificuldade em identificar corretamente todos os óbitos.",styles["BodyText"]))
            story.append(Paragraph("Isso pode ser explicado pela complexidade dos dados, possivelmente existindo relações não lineares entre as variáveis, bem como pelo desequilíbrio entre as classes. Mesmo o Random Forest, que apresentou os melhores resultados, mostrou-se limitado na identificação da classe minoritária.",styles["BodyText"]))
            story.append(Paragraph("Dessa forma, para esse conjunto de dados, a matriz de confusão se mostrou essencial para uma análise adequada dos resultados, demonstrando que a accuracy, por si só, pode encobrir problemas relevantes do ponto de vista clínico.",styles["BodyText"]))
