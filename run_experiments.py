"""Run ML classification experiments and export metrics.

Outputs
-------
outputs/results_long.csv
    One row per (dataset, test_size, preprocessing, model) with accuracy, precision,
    recall, f1 and confusion-matrix entries.
outputs/results_summary.csv
    Pivoted summary with accuracy and precision (weighted) for quick comparison.
outputs/confusion_matrices/*.png
    Confusion matrix image for every run.

Notes
-----
The assignment mentions "labelEncoder" variants. In these datasets, the target is
already numeric/binary, but we still apply LabelEncoder to the target for
standardization. For features, OneHotEncoder is applied to detected categorical
(object/category) columns. For purely numeric datasets, one-hot has no effect.
"""
